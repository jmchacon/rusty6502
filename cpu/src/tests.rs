        ...
        mod $suite {
            ...
            $(
                ...
                fn $name() -> Result<()> {
                    ...
                    let got = wrapped_cpu.borrow().pc.0;
                    tester!(got == want, d, "{state}: Got wrong PC {got:#06X} want {want:#06X}");
                    ...
                    }
                    {
                        ...
                        tester!(i == &InterruptStyle::None, d, "{state}: IRQ wasn't cleared after BRK - {i:?}");
                    }
                    tester!(
                        wrapped_cpu.borrow().interrupt_state == InterruptState::None,
                        d,
...
irq_and_nmi_test!(
    irq_and_nmi_tests,
    nmos: &IrqAndNMI{cpu_type: Type::NMOS}
    cmos: &IrqAndNMI{cpu_type: Type::CMOS}
);
...
#[allow(clippy::too_many_lines)]
fn run_rom_test(r: &RomTest, dbg: bool) -> Result<()> {
    let mut cpu: Cpu<'_>;
    ...
    if dbg {
        cpu = setup(r.cpu, 0x0202, 0x00, None, None, None, Some(&debug));
    } else {
        cpu = setup(r.cpu, 0x0202, 0x00, None, None, None, None);
    }
    cpu.power_on()?;
        ...
        ($suite:ident, $($name:ident: $rom_test:expr)*) => {
            mod $suite {
                ...
                $(
                    ...
                    fn $name() -> Result<()> {
                        let r = $rom_test;
                        let res = panic::catch_unwind(|| {run_rom_test(&r, false)});
                        if res.is_err() {
