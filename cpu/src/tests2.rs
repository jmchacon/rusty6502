use crate::{
    disassemble, AddressMode, CPUError, CPUImpl, CPUInternal, CPUNmosInternal, CPUState, CpuCmos,
    CpuNmos, CpuNmos6510, CpuRicoh, Flags, FlatRAM, InstructionMode, InterruptState,
    InterruptStyle, NewChipDef, OpState, Opcode, Register, State, Tick, Vectors, P_B, P_CARRY,
...
fn disassemble_test() -> Result<()> {
    ...
    for t in &tests {
        ...
        let (d, _) = disassemble::step2(&cpu, addr, cpu.ram.borrow().as_ref());
