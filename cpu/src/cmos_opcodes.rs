...
pub(crate) fn cmos_wdc_opcodes() -> &'static AHashMap<Opcode, AHashMap<AddressMode, Vec<u8>>> {
    static CMOS_WDC_OPCODES: OnceLock<AHashMap<Opcode, AHashMap<AddressMode, Vec<u8>>>> =
        OnceLock::new();
    CMOS_WDC_OPCODES.get_or_init(|| {
        let m = AHashMap::from([
            (
                Opcode::ADC,
                AHashMap::from([
...
// to an Opcode.
pub(crate) fn cmos_wdc_opcodes_values() -> &'static Vec<Operation> {
    static CMOS_WDC_OPCODES_VALUES: OnceLock<Vec<Operation>> = OnceLock::new();
    CMOS_WDC_OPCODES_VALUES.get_or_init(|| {
...
// where the index in the vec indicates the bit it is impacting.
#[allow(clippy::too_many_lines)]
pub(crate) fn cmos_rockwell_opcodes() -> &'static AHashMap<Opcode, AHashMap<AddressMode, Vec<u8>>> {
    static CMOS_ROCKWELL_OPCODES: OnceLock<AHashMap<Opcode, AHashMap<AddressMode, Vec<u8>>>> =
        OnceLock::new();
    CMOS_ROCKWELL_OPCODES.get_or_init(|| {
        let m = AHashMap::from([
            (
                Opcode::ADC,
                AHashMap::from([
...
// the Opcode and AddressMode. Used in processing the CPU tick() or in disassembly for mapping a byte code back
// to an Opcode.
pub(crate) fn cmos_rockwell_opcodes_values() -> &'static Vec<Operation> {
    static CMOS_ROCKWELL_OPCODES_VALUES: OnceLock<Vec<Operation>> = OnceLock::new();
    CMOS_ROCKWELL_OPCODES_VALUES.get_or_init(|| {
        // We know this much be a vector of all u8 values since the 6502
        // has behavior at each so we'll have some combo of opcode/addressmode.
        //
        // Preallocate a vector of that size and fill with placeholders.
...
pub(crate) fn cmos_65SC02_opcodes() -> &'static AHashMap<Opcode, AHashMap<AddressMode, Vec<u8>>> {
    static CMOS_C65SC02_OPCODES: OnceLock<AHashMap<Opcode, AHashMap<AddressMode, Vec<u8>>>> =
        OnceLock::new();
    CMOS_C65SC02_OPCODES.get_or_init(|| {
        let m = AHashMap::from([
            (
                Opcode::ADC,
                AHashMap::from([
...
pub(crate) fn cmos_65SC02_opcodes_values() -> &'static Vec<Operation> {
    static CMOS_65SC02_OPCODES_VALUES: OnceLock<Vec<Operation>> = OnceLock::new();
    CMOS_65SC02_OPCODES_VALUES.get_or_init(|| {
        // We know this much be a vector of all u8 values since the 6502
        // has behavior at each so we'll have some combo of opcode/addressmode.
        //
        // Preallocate a vector of that size and fill with placeholders.
